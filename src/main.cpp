...
int main()
{
  ...
  // using MatrixType = MyMatrixX<float>;
  ...
  // bbcg<MatrixType, VectorType>(A, b,3,2, x, 10e-16);
  // std::cout << x << "\n"; 
  ...
  VectorType c2 = rhs.col(500);
  VectorType c3 = rhs.col(200);
  VectorType c4 = rhs.col(100);

  RHSType B(rhs.rows(), 4);// = rhs.leftCols(2);//
  B.col(0) = c1;
  B.col(1) = c2;
  B.col(2) = c3;
  B.col(3) = c4;
