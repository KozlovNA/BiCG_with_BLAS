...
//BCGSTAB
//---------------------------------------------
// using extended types for dot products
template<class MT, class VT>
void ebcg (const MT      &A,
                   const VT      &b,
                   VT            &x,
                   const double  &eps)
{
  ...
  std::ofstream logs("../output/EBCGSTAB_logs.csv", std::ios::out | std::ios::trunc);
  logs << "k,res_2norm_rel,matvec_count\n";
  ...
  {
    A.template matvec<std::vector<T>, std::vector<T>>(pk, n, vk);
    betak_denom = edotc<T>(n, r0c.data(),1, rk.data(),1);
    alphak = betak_denom /
             edotc<T>(n, r0c.data(),1, vk.data(),1);
    BLAS::axpy(n, -alphak,vk.data(),1, rk.data(),1);
    ...
    A.template matvec<std::vector<T>, std::vector<T>>(rk, n, tk);
    omegak = edotc(n, tk.data(),1, rk.data(),1) /
             edotc(n, tk.data(),1, tk.data(),1);
    BLAS::axpy(n, alphak,pk.data(),1, x.data(),1);
    BLAS::axpy(n, omegak,rk.data(),1, x.data(),1);
    ...
    betak = edotc<T>(n, r0c.data(),1, rk.data(),1) * static_cast<eT>(alphak) /
            betak_denom / static_cast<eT>(omegak);
    BLAS::scal(n, betak,pk.data(),1);
    BLAS::axpy(n, one,rk.data(),1, pk.data(),1);
    BLAS::axpy(n, -betak*omegak,vk.data(),1, pk.data(),1);
  }
